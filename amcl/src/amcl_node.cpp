...
class AmclNode
{
  ...
  private:
    ...
    //对已发布变换的容差时间，基本上定义了map-> odom变换有多长时间
    ...
    boost::recursive_mutex configuration_mutex_;    // 为了按顺序配置，所以需要锁
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    odom_->UpdateAction(pf_, (AMCLSensorData*)&odata);   // 更新运动模型
  ...
  if(lasers_update_[laser_index])
  {
    ...
    lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&ldata); // 更新测量值
    ...
    {
      // 重采样使用了KLD算法，并对采样后的粒子进行聚类，然后找出权重最大的类，把这个类当作当前机器人的位置
