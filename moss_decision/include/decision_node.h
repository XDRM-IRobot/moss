...
#include <moss_msgs/share_info.h>
...
public:
    decision_node(): ac_("move_base", true)
    {
        ...
        sub_car      = nh.subscribe<moss_msgs::car_info>("car_info", 5, boost::bind(&decision_node::car_callback, this, _1));
        ...
        sub_armor    = nh.subscribe<moss_msgs::armor_info>("armor_info", 5, boost::bind(&decision_node::enemy_callback, this, _1));
        sub_network  = nh.subscribe<moss_msgs::share_info>("team_info", 5, boost::bind(&decision_node::multiwar_callback, this, _1));

        pub_goal     = nh.advertise<move_base_msgs::MoveBaseGoal>("nav_goal", 10);
        pub_shoot    = nh.advertise<moss_msgs::shoot_info>("shoot_info", 10);
        pub_network  = nh.advertise<moss_msgs::share_info>("my_info", 10);
        pub_goal.publish(goal);
        ac_.waitForServer();
    }

    void car_callback(const moss_msgs::car_info::ConstPtr & info)
    {
        ; // 
    }
    ...
    void enemy_callback(const moss_msgs::armor_info::ConstPtr & enemy)
    {
        // ac_.sendGoal(goal);
    ...
    void multiwar_callback(const moss_msgs::share_info::ConstPtr & share_msg)
    {
        ; // todo
    }
